---
name: gitlab-wf
description: Handles all GitLab and local git operations for the current repository using the git and glab CLIs. Every action is presented for user confirmation before execution. Activated by git-wf; also invoke directly for GitLab-specific operations.
metadata:
  skill-type: workflow
  version: "1.0"
  memory-file: workflow/gitlab.md
...
| File | Load when |
|---|---|
| `subflows/gitlab-mr-sf.md` | User wants to create, review, merge, or close an MR |
| `subflows/gitlab-pipeline-sf.md` | User wants to trigger, monitor, retry, or cancel a pipeline |
